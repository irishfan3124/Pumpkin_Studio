...
2. **Customize text or an upload.** Typed text offers Bold block, Rounded, Classic serif, and Typewriter styles, plus a 50–150% font-size slider. Spaces and line breaks are supported, and the app adds small stencil bridges to enclosed letter shapes such as O and B. For an upload, use **Cut out light areas instead** when the light parts should become holes, and adjust **Image threshold** to clean up the silhouette. Blank image margins are cropped automatically. **Rotate design** works with typed text and uploads.

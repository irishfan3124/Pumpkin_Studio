...
function useTextDesign(){
 ...
 if(!value){textDesign=false;contours=presets.blank;$('imageRotation').disabled=true;$('rotation-hint').hidden=false;$('upload-name').textContent='Type a message to turn it into cutouts.';queue();return;}
 try{contours=makeTextContours(value);textDesign=true;$('imageRotation').disabled=false;$('rotation-hint').hidden=true;$('upload-name').textContent='Text cutout: “'+value+'”';for(const b of document.querySelectorAll('[data-face]')){b.classList.remove('active');b.setAttribute('aria-pressed','false');}queue();}
...
async function loadFile(file){if(!file)return;const token=++uploadToken;try{if(file.size>10*1024*1024)throw Error('Please choose a file under 10 MB.');if(!/\.(svg|png|jpe?g|webp)$/i.test(file.name))throw Error('Choose an SVG, PNG, JPG, or WebP file.');let blob=file;
 ...
 const url=URL.createObjectURL(blob);const img=new Image();try{img.src=url;await img.decode();if(img.naturalWidth>12000||img.naturalHeight>12000)throw Error('Please resize the image below 12,000 pixels per side.');if(token!==uploadToken)return;uploadImage=img;textDesign=false;$('faceText').value='';maskImage();}finally{URL.revokeObjectURL(url);}

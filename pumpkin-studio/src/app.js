...
function queue(){revision++;exportButtons.forEach(b=>b.disabled=true);status.classList.remove('error');status.textContent='Carving your pumpkin…';clearTimeout(timer);timer=setTimeout(()=>worker.postMessage({id:revision,params:{...state,textDesign,textWrap:$('textWrap').checked},contours:uploadImage||textDesign?rotateUploadedFace(contours,state.width*.65/(state.height*.51),state.imageRotation):contours}),220);}
...
function useTextDesign(){
 ...
 if(!value){textDesign=false;setTextControls(false);$('textWrap').checked=false;state.textScale=defaults.textScale;labels();contours=presets.blank;$('imageRotation').disabled=true;$('rotation-hint').hidden=false;$('upload-name').textContent='Type a message to turn it into cutouts.';queue();return;}
...
$('textWrap').addEventListener('change',queue);
let uploadToken=0;
async function loadFile(file){if(!file)return;const token=++uploadToken;try{if(file.size>10*1024*1024)throw Error('Please choose a file under 10 MB.');if(!/\.(svg|png|jpe?g|webp)$/i.test(file.name))throw Error('Choose an SVG, PNG, JPG, or WebP file.');let blob=file;
 ...
 const url=URL.createObjectURL(blob);const img=new Image();try{img.src=url;await img.decode();if(img.naturalWidth>12000||img.naturalHeight>12000)throw Error('Please resize the image below 12,000 pixels per side.');if(token!==uploadToken)return;uploadImage=img;textDesign=false;setTextControls(false);$('textWrap').checked=false;state.textScale=defaults.textScale;labels();$('faceText').value='';maskImage();}finally{URL.revokeObjectURL(url);}
